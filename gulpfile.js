...
const constant = {
  ...
  SRC_IMG_PATH: "src/img",
  SRC_SCSS_PATH: "src/scss",
  SRC_JS_PATH: "src/js",
  BUILD_PATH: "build",
  BUILD_HTML_PATH: "build",
  BUILD_IMG_PATH: "build/assets/img",
...
function copyImagesTask() {
  return gulp
    .src(constant.SRC_IMG_PATH + "/**/*")
    .pipe(gulp.dest(constant.BUILD_IMG_PATH));
}
...
function watchTask() {
  ...
  gulp.watch(constant.SRC_IMG_PATH + "/**/*", copyImagesTask);
  gulp.watch(constant.SRC_PATH + "/**/*", browserSyncReload);
}

const build = gulp.series(
  cleanTask,
  gulp.parallel(cssTask, jsTask, copyHTMLTask, copyImagesTask)
);
